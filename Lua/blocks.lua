function effectblock()
	...
	if (featureindex["level"] ~= nil) then
		...
		local isnavy = hasfeature("level","is","navy",1)
		local ismaroon = hasfeature("level","is","maroon",1)
		local isgold = hasfeature("level","is","gold",1)
		local isblurple = hasfeature("level","is","blurple",1)
		local iscerulean = hasfeature("level","is","cerulean",1)
	
		local colours = {isred, isorange, isyellow, islime, isgreen, iscyan, isblue, ispurple, ispink, isrosy, isblack, isgrey, issilver, iswhite, isbrown, isnavy, ismaroon, isgold, isblurple, iscerulean}
		local ccolours = {{2,2},{2,3},{2,4},{5,3},{5,2},{1,4},{3,2},{3,1},{4,1},{4,2},{0,4},{0,1},{0,2},{0,3},{6,1},{6,2},{3,3},{4,3}}
	...
	for i,unit in ipairs(units) do
		...
		if (unit.className ~= "level") then		
			...
			local isnavy = hasfeature(name,"is","navy",unit.fixed)
			local ismaroon = hasfeature(name,"is","maroon",unit.fixed)
			local isgold = hasfeature(name,"is","gold",unit.fixed)
			local isblurple = hasfeature(name,"is","blurple",unit.fixed)
			local iscerulean = hasfeature(name,"is","cerulean",unit.fixed)
			
			unit.colours = {}
			
			local colours = {isred, isorange, isyellow, islime, isgreen, iscyan, isblue, ispurple, ispink, isrosy, isblack, isgrey, issilver, iswhite, isbrown, isnavy, ismaroon, isgold, isblurple, iscerulean}
			local ccolours = {{2,2},{2,3},{2,4},{5,3},{5,2},{1,4},{3,2},{3,1},{4,1},{4,2},{0,4},{0,1},{0,2},{0,3},{6,1},{1,1},{2,1},{6,2},{3,3},{4,3}}
...
function statusblock(ids,undoing_,noturn_)
	...
	for i,unit in pairs(checkthese) do
		...
		if (undoing == false) then
			...
			if (hasfeature_count(name,"is","kata",unit.fixed) > 0) then
				fdir = 4
				updatedir(unit.fixed,fdir)
			end
...
function block(small_)
	...
	local istouchy = getunitswitheffect("crumble",false,delthese)
	...
	for id,unit in ipairs(istouchy) do
		if (issafe(unit.fixed) == false) and (unit.new == false) then
			local x,y = unit.values[XPOS],unit.values[YPOS]
			local stuff = findallhere(x,y)
			...
			if (#stuff > 0) then
				for i,v in ipairs(stuff) do
					if floating(v,unit.fixed,x,y) then
						...
						local thistype = vunit.strings[UNITTYPE]
						if (v ~= unit.fixed) then
							local pmult,sound = checkeffecthistory("crumble")
							MF_particles("destroy",x,y,5 * pmult,0,3,1,1)
							removalshort = sound
							removalsound = 9
							generaldata.values[SHAKE] = 4
	...
	-- this code right here is really lazy
	local ismlelt = getunitswitheffect("mlelt",false,delthese)
	for id,unit in ipairs(ismlelt) do
		local hot = findfeature(nil,"is","hot")
		local x,y = unit.values[XPOS],unit.values[YPOS]
