	...
	while (take <= takecount) or finaltake do
		...
		if (finaltake == false) then
			if (take == 1) then
				...
				playerwasdturning = findallfeature(nil,"is","wasd")
				...
				for i,v in ipairs(playerwasdturning) do
					
					fdir = dir_
					--[[
					if (playerid == 3) then
						if (i > #playerwasdturning - #playerwasdturning2) then
							fdir = dir_2
						end
					end
					--]]
									...
									if (state < 2) then
										...
										if (state < 3) then
											...
											if ((data.reason == "move") or (data.reason == "chill")) and (state < 4) then
												...
												local splat = hasfeature(name,"is","splat",data.unitid,x,y)
												local bounce = hasfeature(name,"is","bounce",data.unitid,x,y)
												
												if (weak ~= nil) or (splat ~= nil) and (issafe(data.unitid,x,y) == false) then
	...
	if (gone == false) and (simulate == false) and (unitid ~= 2) then
		...
		if unit.visible and (#movelist < 700) and (spritedata.values[VISION] == 0) then
			...
			if (unit.values[TILING] == 2) then
				...
				local kata = hasfeature(unitname,"is","kata",v)
				if kata then
					unit.values[VISUALDIR] = 4
				end
			end
		end
	end
